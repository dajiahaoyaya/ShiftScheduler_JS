...
const NightShiftManager = {
    ...
    /**
     * 为完成排班被取消的特殊休假 [{ staffId, dateStr }]，表格中这些日期按大夜显示
     */
    currentRemovedVacations: [],
    ...
    getConfigLockKey(config) {
        if (typeof Store !== 'undefined' && Store && typeof Store.resolveConfigLockKey === 'function') {
            return Store.resolveConfigLockKey(config, { configType: 'nightShift' });
        }
        return null;
    },
    ...
    isConfigInActiveLock(config) {
        if (typeof Store !== 'undefined' && Store && typeof Store.isConfigInActiveLock === 'function') {
            return Store.isConfigInActiveLock(config, { configType: 'nightShift' });
        }
        return false;
    },

    findExistingConfigInActiveLock(excludeConfigId = null) {
        const configs = Store.getNightShiftConfigs ? (Store.getNightShiftConfigs() || []) : [];
    ...
    downloadArchiveSnapshot(config, prefix = 'night-shift-archive') {
    ...
    renderArchiveReadonly(config) {
        ...
        const looksLikeDateMap = scheduleKeys.length > 0 && Array.isArray(schedule[scheduleKeys[0]]);
        if (looksLikeDateMap) {
            scheduleKeys.forEach((dateStr) => {
                const staffList = Array.isArray(schedule[dateStr]) ? schedule[dateStr] : [];
                dateMap[dateStr] = staffList.map((item) => {
                    if (!item) return '';
                    if (typeof item === 'string') return item;
                    return item.staffId || item.id || item.name || '';
                }).filter(Boolean);
            });
        } else {
            scheduleKeys.forEach((staffId) => {
                ...
                Object.keys(staffSchedule).forEach((dateStr) => {
                    ...
                    if (!dateMap[dateStr].includes(staffId)) {
                        dateMap[dateStr].push(staffId);
                    }
                });
            });
        }
        const rowsHtml = Object.keys(dateMap).sort().map((dateStr) => {
            const staffList = dateMap[dateStr] || [];
            const staffText = staffList.join('、');
            const keyword = `${dateStr} ${staffText}`.toLowerCase();
            return `
                <tr data-archive-keyword="${esc(keyword)}" class="hover:bg-gray-50">
                    <td class="px-3 py-2 text-xs text-gray-900 border border-gray-200">${esc(dateStr)}</td>
                    <td class="px-3 py-2 text-xs text-gray-900 border border-gray-200 font-medium">${staffList.length}</td>
                    <td class="px-3 py-2 text-xs text-gray-600 border border-gray-200">${esc(staffText)}</td>
                </tr>
            `;
        }).join('');
        ...
        scheduleTable.innerHTML = `
            <div class="p-6 space-y-4">
                ...
                <div class="flex items-center gap-3">
                    <button onclick="NightShiftManager.showNightShiftManagement()" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 text-sm font-medium">返回配置列表</button>
                    <button onclick="NightShiftManager.downloadArchiveSnapshot(Store.getNightShiftConfig('${config.configId}'))" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium">导出JSON</button>
                </div>
                <div class="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
                    <input id="night-archive-filter" type="text" placeholder="筛选：日期/人员" oninput="NightShiftManager.filterArchiveTable(this.value)" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md">
                    <div class="overflow-x-auto overflow-y-auto" style="max-height: 60vh;">
                        <table class="min-w-full border-collapse">
                            <thead class="sticky top-0 bg-gray-50 z-10">
                                <tr>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 border border-gray-200">日期</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 border border-gray-200">大夜人数</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 border border-gray-200">人员列表</th>
                                </tr>
                            </thead>
                            <tbody id="night-archive-tbody">
                                ${rowsHtml || '<tr><td colspan="3" class="px-3 py-6 text-center text-sm text-gray-500 border border-gray-200">暂无大夜排班数据</td></tr>'}
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    filterArchiveTable(keyword) {
        const tbody = document.getElementById('night-archive-tbody');
        if (!tbody) return;
    ...
    async chooseCityScope(actionLabel = '新建大夜配置', defaultScope = 'ALL') {
        ...
        return new Promise((resolve) => {
            ...
            dialog.innerHTML = `
                ...
                <p class="text-sm text-gray-600 mb-3">首次生成请选择城市范围，创建后该配置的城市范围将锁定。</p>
                <select id="night-city-scope-select" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm mb-5">
                ...
                <div class="flex justify-end space-x-3">
                    <button id="night-city-scope-cancel" class="px-4 py-2 rounded bg-gray-200 text-gray-700 hover:bg-gray-300">取消</button>
                    <button id="night-city-scope-ok" class="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">确定</button>
                </div>
            `;
            ...
            const cancelBtn = dialog.querySelector('#night-city-scope-cancel');
            const okBtn = dialog.querySelector('#night-city-scope-ok');
            const selectEl = dialog.querySelector('#night-city-scope-select');
    ...
    async calculateManpowerSufficiency(regionKey, dateRange) {
        ...
        try {
            ...
            if (typeof DailyManpowerManager !== 'undefined' && DailyManpowerManager.matrix) {
                ...
                const activeDailyManpowerConfigId = (typeof Store !== 'undefined' && typeof Store.getState === 'function')
                    ? Store.getState('activeDailyManpowerConfigId')
                    : null;
                if (dailyMax === 2 && activeDailyManpowerConfigId) {
                    const activeId = activeDailyManpowerConfigId;
     ...
     * @param {number|string|null} randomSeed - 随机数种子，相同种子会产生相同结果
     * @returns {Object} 排班结果和统计信息
     */
    async generateNightShiftSchedule(dateRange, config = null, randomSeed = null) {
        ...
        try {
            ...
            // 检查 IncrementalNightShiftSolver 是否加载
            const incrementalSolverExists = typeof IncrementalNightShiftSolver !== 'undefined';

            if (!incrementalSolverExists) {
                throw new Error('IncrementalNightShiftSolver 未加载，无法生成大夜排班。请检查 js/solvers/incrementalNightShiftSolver.js 文件是否正确加载。');
            }
            ...
            // 进度条元素已在函数开始处获取，这里确保显示
            if (progressBarContainer && progressBarContainer.classList.contains('hidden')) {
                progressBarContainer.classList.remove('hidden');
            }
            
            let maxIterations = 0; // 用于计算进度百分比（预估）
            
            // 进度回调函数
            const onProgress = (iteration, elapsedSeconds, removedStaffId, removedDateStr) => {
                maxIterations = Math.max(maxIterations, iteration);
                // 估算最大可能迭代次数（假设最多取消所有特殊休假）
                const totalVacations = Object.values(personalRequests).reduce((sum, req) => {
                    if (!req || typeof req !== 'object') return sum;
                    return sum + Object.keys(req).filter(ds => req[ds] && req[ds] !== 'NIGHT').length;
                }, 0);
                const estimatedMax = Math.max(totalVacations, maxIterations);
                const progressPercent = estimatedMax > 0 ? Math.min(100, (iteration / estimatedMax) * 100) : 0;
                ...
                // 构建进度文本和状态栏消息
                let progressTextMsg = `策略循环 ${iteration} 次，耗时 ${elapsedSeconds} 秒`;
                let statusMsg = `正在生成大夜排班... (策略循环 ${iteration} 次，耗时 ${elapsedSeconds} 秒)`;
                
                if (removedStaffId && removedDateStr) {
                    const staff = staffData.find(s => (s.id || s.staffId) == removedStaffId);
                    const staffName = staff ? staff.name : removedStaffId;
                    progressTextMsg += ` | 已取消: ${staffName} (${removedDateStr})`;
                    statusMsg = `正在生成大夜排班... (策略循环 ${iteration} 次) | 已取消 ${staffName} 的 ${removedDateStr} 特殊休假`;
                }
                
                if (progressText) {
                    progressText.textContent = progressTextMsg;
                }
            ...
            // 转换为 NightShiftManager 期望的格式（按日期组织，便于表格渲染）
            const schedule = {};
            ...
            // 2. 补上大夜后的休整期（mandatoryRestDays），并考虑与特殊休假合并后的最大连休上限
            const mandatoryRestDays = incrementalResult.mandatoryRestDays || {};
            const maxConsecutiveRestLimit = this.getMaxConsecutiveRestLimit();
            const postShiftRestDays = (NightShiftConfigRules.getConfig().strictContinuous || {}).postShiftRestDays || 2;
            const dateSet = new Set(dateList);

            const nightScheduleByStaff = incrementalResult.schedule || {};
            for (const [staffId, restDateList] of Object.entries(mandatoryRestDays)) {
                if (!Array.isArray(restDateList)) continue;
                const staff = staffData.find(s => (s.id || s.staffId) == staffId);
                const name = staff ? staff.name : staffId;
                const staffReq = personalRequests[staffId] || personalRequests[String(staffId)] || {};
                const vacationDates = Object.keys(staffReq).filter(d => dateSet.has(d));
                const nightDates = nightScheduleByStaff[staffId] ? Object.keys(nightScheduleByStaff[staffId]).filter(d => nightScheduleByStaff[staffId][d] === 'NIGHT') : [];

                const toAdd = this.filterMandatoryRestByConsecutiveLimit(
                    restDateList,
                    vacationDates,
                    dateList,
                ...
                toAdd.forEach(dateStr => {
                    if (schedule[dateStr]) {
                        // 【修复】检查该员工在该日期是否已经有夜班记录
                        // 如果已有夜班，不添加休整期记录（避免同一天既有夜班又有休整）
                        const existingAssignment = schedule[dateStr].find(
                        ...
                        } else if (existingAssignment.shiftType !== 'night') {
                            // 如果有记录但不是夜班，也添加休整期
                            schedule[dateStr].push({
                                staffId: staffId,
                                name: name,
            ...
            // 保存到实例变量（表格渲染会使用：大夜 + 休整期；个性化休假由 renderScheduleTableInResults 从 Store 读取并叠加显示）
            this.currentSchedule = schedule;
            this.currentRemovedVacations = incrementalResult.removedVacations || [];

            // 持久化到数据库
            // 【修复】保存完整格式（包含大夜和休整期），供月度班次配置等模块读取
            await DB.saveNightShiftSchedule({
                scheduleId: 'current',
                schedule: schedule, // 保存完整格式（大夜 + 休整期）
                stats: incrementalResult.stats,
                dateRange,
                createdAt: new Date().toISOString()
            });

            // 为排行/人力展示补全 stats.shanghai（与 Legacy 格式一致）
            const totalDays = dateList.length;
            const staffNightShiftCounts = incrementalResult.stats.staffNightShiftCounts || {};
            let maleAssignments = 0;
            let femaleAssignments = 0;
            for (const [sid, count] of Object.entries(staffNightShiftCounts)) {
                const s = staffData.find(staff => (staff.id || staff.staffId) === sid);
                if (s && (s.gender === '女' || s.gender === 'F')) femaleAssignments += count;
                else maleAssignments += count;
            }
            const stats = {
                ...incrementalResult.stats,
                shanghai: {
                    totalAssignments: incrementalResult.stats.totalNightShifts || 0,
                    maleAssignments,
                    femaleAssignments,
                    dailyAverage: totalDays > 0 ? (incrementalResult.stats.totalNightShifts / totalDays).toFixed(2) : '0.00'
                }
            };
            ...
            const result = {
                ...
                removedVacations: this.currentRemovedVacations
            };
            ...
            // 确保进度条隐藏（即使出错）
            const errorProgressBarContainer = document.getElementById('progressBarContainer');
            const errorProgressBarFill = document.getElementById('progressBarFill');
            if (errorProgressBarContainer) errorProgressBarContainer.classList.add('hidden');
            if (errorProgressBarFill) errorProgressBarFill.style.width = '0%';
    ...
    /**
     * 获取最大连休上限（休整期+特殊休假合并后不可超过此天数）
     * 优先从大夜配置 strictContinuous.maxConsecutiveRestLimit 读取，0 表示从排班周期管理获取
     */
    getMaxConsecutiveRestLimit() {
        const strictConfig = NightShiftConfigRules.getConfig().strictContinuous || {};
        let max = strictConfig.maxConsecutiveRestLimit;
        if (max === 0 && typeof SchedulePeriodManager !== 'undefined') {
            try {
                const activeId = Store.getState('activeSchedulePeriodConfigId');
                if (activeId) {
                    const periodConfig = Store.getSchedulePeriodConfig(activeId);
                    if (periodConfig && typeof periodConfig.maxConsecutiveRestDays === 'number') {
                        max = periodConfig.maxConsecutiveRestDays;
                    }
                }
            } catch (e) {}
        }
    ...
    /**
     * 在满足「休整期最少 minMandatory 天、最大连休上限 maxLimit」前提下，过滤出可加入的休整期日期
     * 大夜后的休整+特殊休假：最小值为休整期（当前2天），最大值为 maxLimit
     * @param {string[]} mandatoryRestList - 该员工大夜后强制休整日期列表
     * @param {string[]} vacationDates - 该员工特殊休假日期列表（在周期内）
     * @param {string[]} dateList - 周期内所有日期（有序）
     * @param {number} maxLimit - 最大连休上限
     * @param {number} minMandatory - 休整期最少保留天数（如 2）
     * @param {string[]} [nightDates] - 该员工大夜日期列表，用于计算「每夜后 2 天」受保护休整日
     * @returns {string[]} 实际可加入排班表的休整期日期
     */
    filterMandatoryRestByConsecutiveLimit(mandatoryRestList, vacationDates, dateList, maxLimit, minMandatory = 2, nightDates = []) {
        if (!maxLimit || maxLimit >= 99) return mandatoryRestList;
        const allRestSet = new Set([...mandatoryRestList, ...vacationDates]);
        const sorted = dateList.filter(d => allRestSet.has(d)).sort();
        if (sorted.length === 0) return mandatoryRestList;

        const mandatorySet = new Set(mandatoryRestList);
        const dateIndex = new Map(dateList.map((d, idx) => [d, idx]));
        const protectedRestDates = new Set();
        for (const nightStr of nightDates) {
            const idx = dateIndex.get(nightStr);
            if (idx == null) continue;
            if (dateList[idx + 1]) protectedRestDates.add(dateList[idx + 1]);
            if (dateList[idx + 2]) protectedRestDates.add(dateList[idx + 2]);
        }
        ...
        while (i < sorted.length) {
            ...
            while (j < sorted.length) {
                ...
                if (segment.length > 0) {
                    ...
                    if (prevDate.toISOString().slice(0, 10) !== d) break;
                }
                segment.push(d);
                j++;
            }
            const mandatoryInSegment = segment.filter(d => mandatorySet.has(d));
            const vacationInSegment = segment.length - mandatoryInSegment.length;
            const protectedInSegment = mandatoryInSegment.filter(d => protectedRestDates.has(d));
            const otherMandatory = mandatoryInSegment.filter(d => !protectedRestDates.has(d));
            const allowedMandatory = Math.min(
                mandatoryInSegment.length,
                Math.max(minMandatory, maxLimit - vacationInSegment)
            );
            const totalToAdd = Math.min(mandatoryInSegment.length, Math.max(protectedInSegment.length, allowedMandatory));
            protectedInSegment.forEach(d => toAdd.add(d));
            const extra = totalToAdd - protectedInSegment.length;
            if (extra > 0 && otherMandatory.length > 0) {
                otherMandatory.sort();
                otherMandatory.slice(0, extra).forEach(d => toAdd.add(d));
            }
            i = j;
        }

        return mandatoryRestList.filter(d => toAdd.has(d));
    },
    ...
    async renderConfigList(container) {
        ...
        try {
            ...
            if (dbConfigs && dbConfigs.length > 0) {
                // 同步到 Store
                if (typeof Store.replaceNightShiftConfigs === 'function') {
                    Store.replaceNightShiftConfigs(dbConfigs, false, { actorEmpNo: 'SYSTEM_MIGRATION' });
                    configs = Store.getNightShiftConfigs ? Store.getNightShiftConfigs() : dbConfigs;
                } else {
                    Store.updateState({ nightShiftConfigs: dbConfigs }, false);
                    configs = dbConfigs;
                }
        ...
        if (activeConfigId) {
            const activeNightConfig = configs.find((c) => c && c.configId === activeConfigId);
            if (activeNightConfig && !currentYearMonth && activeNightConfig.scheduleConfig) {
                currentYearMonth = `${activeNightConfig.scheduleConfig.year}${String(activeNightConfig.scheduleConfig.month).padStart(2, '0')}`;
            }
        }
        ...
        if (filteredConfigs.length === 0) {
            html += `
                <div class="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <p class="text-gray-500 mb-4">${canCreate ? `当前激活城市为${this.getCityScopeLabel(chainCityScope)}，可创建当前锁大夜配置` : createHint}</p>
                    <button onclick="NightShiftManager.createNewConfig()"
                        ${canCreate ? '' : 'disabled'}
                        title="${createHintEscaped}"
            ...
            sortedConfigs.forEach((config) => {
                ...
                const rowOperateHint = rowOperateAllowed
                    ? ''
                    : (!chainContext.ok ? createHint : '归档配置仅支持查看，不可编辑/导入/激活');
                    ...
                    <tr class="${isActive ? 'bg-blue-50' : ''}">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                ` : `
                                    <button onclick="NightShiftManager.deactivateConfig()"
    ...
    async createNewConfig() {
        try {
            ...
            if (existing) {
                alert(`当前激活锁已存在大夜配置：${existing.name}。请先删除后再新建。`);
                return;
            }
            ...
            const configId = Store.createNightShiftConfig(
                name,
                currentNightShiftConfig,
                {
            ...
            const configData = Store.getNightShiftConfig(configId);
            if (!configData) {
                throw new Error('创建大夜配置后读取失败');
            }
            ...
            await Store.setActiveCityScope(targetCityScope, false);
    ...
    async viewConfigEntry(configId) {
        ...
        try {
            // 获取配置
            const config = Store.getNightShiftConfig
                ? Store.getNightShiftConfig(configId)
                : null;
            ...
            // 【修复】检测并转换 schedule 格式
            if (config.schedule) {
                const firstKey = Object.keys(config.schedule)[0];
                const firstValue = config.schedule[firstKey];
                const isDateFormat = Array.isArray(firstValue);
                
                if (isDateFormat) {
                    // 已经是日期格式（完整格式），直接使用
                    this.currentSchedule = config.schedule;
                    console.log('[NightShiftManager] config.schedule 是日期格式，直接使用');
                } else {
                    // 是员工格式（旧格式），需要转换为日期格式
                    console.log('[NightShiftManager] config.schedule 是员工格式，进行转换...');
                    const convertedSchedule = {};
                    Object.keys(config.schedule).forEach(staffId => {
                        const staffSchedule = config.schedule[staffId];
                        if (staffSchedule && typeof staffSchedule === 'object') {
                            Object.keys(staffSchedule).forEach(dateStr => {
                                const shiftValue = staffSchedule[dateStr];
                                if (!convertedSchedule[dateStr]) {
                                    convertedSchedule[dateStr] = [];
                                }
                                const isRest = shiftValue === 'rest';
                                convertedSchedule[dateStr].push({
                                    staffId: staffId,
                                    shiftType: isRest ? 'rest' : 'night',
                                    isPostShiftRest: isRest
                                });
                            });
                        }
                    });
                    this.currentSchedule = convertedSchedule;
                    console.log('[NightShiftManager] 转换完成，日期数量:', Object.keys(convertedSchedule).length);
                }
            ...
            <div class="p-4 border-b border-gray-200 bg-white">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center space-x-2" id="nightShiftActionButtons">
                        <div class="flex items-center space-x-2">
                            <label class="text-sm text-gray-600 whitespace-nowrap">随机数种子:</label>
                            <input type="number" id="randomSeedInput" 
                ...
                <div class="text-xs text-gray-500">
                    ...
                    <p>城市范围: ${this.getCityScopeLabel(this.getConfigCityScope(config))}（已锁定）</p>
                </div>
            </div>
    ...
    async updateConfigName() {
        ...
        const config = Store.getNightShiftConfig
            ? Store.getNightShiftConfig(this.currentConfigId)
            : null;

        if (config) {
            if (typeof Store.updateNightShiftConfig === 'function') {
                Store.updateNightShiftConfig(this.currentConfigId, { name: newName });
            } else {
                config.name = newName;
            ...
            // 保存到IndexedDB
            const latestConfig = Store.getNightShiftConfig
                ? Store.getNightShiftConfig(this.currentConfigId)
                : config;
            await DB.saveNightShiftConfigManagement(latestConfig);
            await Store.saveState();
    ...
    async deactivateConfig() {
        if (!Store.getActiveNightShiftConfigId()) {
            alert('当前没有激活的大夜配置');
            return;
        }
        if (!confirm('确定要取消激活当前大夜配置吗？')) {
            return;
        }

        try {
            if (typeof Store.clearActiveNightShiftConfig !== 'function') {
                throw new Error('Store.clearActiveNightShiftConfig 不可用');
            }
            await Store.clearActiveNightShiftConfig();
            await this.renderConfigList(document.getElementById('scheduleTable'));
    ...
    async deleteConfig(configId) {
        ...
        try {
            if (typeof Store.deleteNightShiftConfig !== 'function') {
                throw new Error('Store.deleteNightShiftConfig 不可用');
            }
            Store.deleteNightShiftConfig(configId);
    ...
    async validateAndSaveSchedule() {
        ...
        try {
            ...
            // 2. 更新配置的排班结果
            const nextStats = this.calculateScheduleStats(
                this.currentSchedule,
                Object.keys(this.currentSchedule),
                this.currentManpowerAnalysis
            );
            const updatePayload = {
                ...
                manpowerAnalysis: this.currentManpowerAnalysis,
                // 同步保存当前大夜配置规则快照与排班周期
                nightShiftConfig: NightShiftConfigRules.getConfig()
            };
            const activeSchedulePeriodConfigId = Store.getState('activeSchedulePeriodConfigId');
            if (activeSchedulePeriodConfigId && Store.getSchedulePeriodConfig) {
                const activeSchedulePeriodConfig = Store.getSchedulePeriodConfig(activeSchedulePeriodConfigId);
                if (activeSchedulePeriodConfig && activeSchedulePeriodConfig.scheduleConfig) {
                    updatePayload.scheduleConfig = {
                        startDate: activeSchedulePeriodConfig.scheduleConfig.startDate,
                        endDate: activeSchedulePeriodConfig.scheduleConfig.endDate,
                        year: activeSchedulePeriodConfig.scheduleConfig.year,
                        month: activeSchedulePeriodConfig.scheduleConfig.month
            ...
            if (typeof Store.updateNightShiftConfig === 'function') {
                Store.updateNightShiftConfig(this.currentConfigId, updatePayload);
            } else {
        ...
        saveBtn.onclick = async () => {
            try {
                ...
                if (this.currentConfigId) {
                    const currentConfig = Store.getNightShiftConfig
                        ? Store.getNightShiftConfig(this.currentConfigId)
                        : null;
                    if (currentConfig) {
                        if (typeof Store.updateNightShiftConfig === 'function') {
                            Store.updateNightShiftConfig(this.currentConfigId, {
                                nightShiftConfig: NightShiftConfigRules.getConfig()
                            });
                        } else {
    ...
    async handleGenerateSchedule() {
        try {
            ...
            let dateRange;
            let lockedCityScope = this.normalizeCityScope(Store.getState('activeCityScope'));

            // 方法0：优先使用当前配置（城市范围与周期均锁定）
            if (this.currentConfigId && typeof Store.getNightShiftConfig === 'function') {
                const currentConfig = Store.getNightShiftConfig(this.currentConfigId);
                if (currentConfig) {
                    lockedCityScope = this.getConfigCityScope(currentConfig);
                    if (typeof Store.setActiveCityScope === 'function') {
                        await Store.setActiveCityScope(lockedCityScope, false);
                    }
                    if (currentConfig.scheduleConfig && currentConfig.scheduleConfig.startDate && currentConfig.scheduleConfig.endDate) {
                        dateRange = {
                            startDate: currentConfig.scheduleConfig.startDate,
                            endDate: currentConfig.scheduleConfig.endDate
                        };
                        console.log('[NightShiftManager] 使用当前配置锁定的排班周期:', dateRange, 'scope=', lockedCityScope);
                    }
                }
            }
            ...
            if (!dateRange && activeConfigId && Store.getSchedulePeriodConfig) {
            ...
            if (seedInput && seedInput.value.trim() !== '') {
                ...
                // 尝试转换为数字，如果失败则使用字符串（哈希）
                randomSeed = isNaN(seedValue) ? seedValue : Number(seedValue);
                console.log(`[NightShiftManager] 使用随机数种子: ${randomSeed}`);
            }

            // 生成排班
            const result = await this.generateNightShiftSchedule(dateRange, null, randomSeed);
    ...
    renderScheduleResults(result = null) {
        ...
        // 渲染各个部分（传入被取消的休假，表格中对应日期显示大夜而非休假）
        this.renderScheduleTableInResults(result.schedule, result.removedVacations || this.currentRemovedVacations || []);
     ...
     * @param {Object} schedule - 排班表
     * @param {Array<{staffId, dateStr}>} [removedVacations] - 为完成排班被取消的特殊休假，这些日期按大夜显示
     */
    renderScheduleTableInResults(schedule, removedVacations) {
        ...
        if (!schedule) schedule = this.currentSchedule;
        if (!schedule) {
            container.innerHTML = '<div class="stats-placeholder">暂无排班结果</div>';
            return;
        }
        if (removedVacations === undefined) removedVacations = this.currentRemovedVacations;
        ...
        if (isDateFormat) {
            ...
            // 需要转换：从 { staffId: { dateStr: 'NIGHT'/'rest' } } 转换为 { dateStr: [assignments] }
            transformedSchedule = {};
            Object.keys(schedule).forEach(staffId => {
                ...
                Object.keys(staffSchedule).forEach(dateStr => {
                    ...
                    // 跳过无效值
                    if (!shiftValue || shiftValue === 'NIGHT' || shiftValue === 'rest') {
        ...
        sortedStaffList.forEach((staff, index) => {
            ...
            // 获取该员工的特殊休假配置（兼容 staffId 为数字或字符串的 key）；被取消的休假不显示为休假，该日显示大夜
            const rawVacations = personalRequests[staff.staffId] || personalRequests[String(staff.staffId)] || personalRequests[staff.id] || {};
            const staffVacations = { ...rawVacations };
            const removed = removedVacations || this.currentRemovedVacations || [];
            removed.filter(r => String(r.staffId) === String(staff.staffId)).forEach(r => { delete staffVacations[r.dateStr]; });
            ...
            // 获取被取消的特殊休假日期列表
            const removedVacationsForStaff = (removedVacations || this.currentRemovedVacations || [])
                .filter(r => String(r.staffId) === String(staff.staffId));
            const hasRemovedVacations = removedVacationsForStaff.length > 0;
            const removedDatesText = removedVacationsForStaff.map(r => r.dateStr).join(', ');
            const removedTitle = hasRemovedVacations ? `被取消的特殊休假: ${removedDatesText}` : '';

            // 状态列显示优先级：错误 > 取消休假 > 不可排 > 正常
            let statusIndicator = '';
            if (hasError) {
                statusIndicator = `
                    <span class="inline-flex items-center justify-center w-5 h-5 bg-red-500 rounded-full cursor-help"
                          title="已分配${nightShiftCount}天大夜，超过限制${maxDays}天"
                          style="position: relative;">
                        <span class="text-white text-xs font-bold">!</span>
                    </span>`;
            } else if (hasRemovedVacations) {
                statusIndicator = `
                    <span class="inline-flex items-center justify-center w-5 h-5 bg-amber-500 rounded-full cursor-help"
                          title="${removedTitle}"
                          style="position: relative;">
                        <span class="text-white text-xs font-bold">⚠</span>
                    </span>`;
                ...
                statusIndicator = `
                    <span class="inline-flex items-center justify-center w-5 h-5 bg-gray-400 rounded-full cursor-help"
                          title="不可排大夜"
                          style="position: relative;">
                ...
                statusIndicator = `
                    <span class="inline-flex items-center justify-center w-5 h-5 bg-green-500 rounded-full cursor-help"
                          title="正常"
                          style="position: relative;">
                        <span class="text-white text-xs">✓</span>
                    </span>`;

...
const MonthlyShiftManager = {
    ...
    async createNewConfig() {
        ...
        try {
            ...
            let nightShiftSchedule = null;

            if (activeNightShiftConfigId && Store.state.nightShiftConfigs) {
                const nightShiftConfig = Store.state.nightShiftConfigs.find(c => c.configId === activeNightShiftConfigId);
                if (nightShiftConfig && nightShiftConfig.schedule) {
                    nightShiftSchedule = nightShiftConfig.schedule;
                    console.log('[MonthlyShiftManager] 找到激活的大夜排班配置:', nightShiftConfig.name);
                }
            }
            ...
            staffDataList.forEach(staff => {
                ...
                if (nightShiftSchedule) {
                    Object.keys(nightShiftSchedule).forEach(dateStr => {
                        const daySchedule = nightShiftSchedule[dateStr] || [];
                        const assignment = daySchedule.find(s => s.staffId === staffId);

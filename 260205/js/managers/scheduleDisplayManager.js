...
const ScheduleDisplayManager = {
            ...
            <div class="p-6">
                ...
                ${configs.length === 0 ? `
                    <div class="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <p class="text-gray-500 mb-4">创建您的第一个排班结果配置</p>
                        <button onclick="ScheduleDisplayManager.createNewConfig()"
                                    ...
                                    <div class="flex justify-between items-center pt-3 border-t border-gray-200">
                                        ...
                                        <div class="flex space-x-1">
                                            <button onclick="ScheduleDisplayManager.duplicateConfig('${config.configId}')"
                                                class="p-1.5 text-purple-600 hover:bg-purple-50 rounded" title="复制">
    ...
    async createNewConfig() {
        ...
        const configId = Store.createScheduleResultConfig(configName, scheduleResult, scheduleConfig);
    ...
    async renderScheduleDetail(config) {
        ...
        let html = `
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <h3 class="text-lg font-semibold text-gray-800 mb-4">报表展示</h3>
    ...
    renderScheduleDetailTable(config, dateList, staffData, scheduleResult, monthlyShifts, monthlyDailySchedule, allRestDays, staffScheduleData) {
        ...
        // 排班类型选项
        const SCHEDULE_OPTIONS = ['', '追', '收', '综', '银B', '毛', '星', '网', '天', '微', '大夜', '法定休假', '年休假', '大夜后休整'];
    ...
    updateFilter() {
        if (!this.filterState) {
            this.filterState = {
                ...
                nameFilter: '',
                locations: ['上海', '成都'],
                personTypes: ['正式员工', '外包员工', '授权人员支援侦测', '大夜授权']
            };
        }
    ...
    async generateScheduleDisplay(configId) {
        ...
        try {
            ...
            // 从激活的月度班次配置继承数据
            const sourceStaffScheduleData = activeMonthlyScheduleConfig.staffScheduleData || {};
            const newStaffScheduleData = JSON.parse(JSON.stringify(sourceStaffScheduleData));

            // 转换为排班结果格式
            const scheduleResult = {};
            Object.entries(newStaffScheduleData).forEach(([staffId, staffData]) => {
                if (!scheduleResult[staffId]) {
                    scheduleResult[staffId] = {};
                }
                const dailySchedule = staffData.dailySchedule || {};
            ...
            Store.updateScheduleResultConfig(configId, {
                staffScheduleData: newStaffScheduleData,
                scheduleResultSnapshot: scheduleResult,
                updatedAt: new Date().toISOString()
            }, true);
    ...
    async deactivateConfig() {
        ...
        try {
            Store.updateState({ activeScheduleResultConfigId: null }, false);
            await Store.saveState();
            this.renderConfigList();
            alert('已取消激活');
        } catch (error) {
